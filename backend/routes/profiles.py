...
async def update_bio(jwt_token: str, new_bio: str):
    ...
    await caches.get("profiles").delete(f"profiles_{user_id}")  # Clear the cache to reflect updated bio
    await caches.get("profiles").delete("all_profiles")  # Clear the cache for get all profiles
...
async def update_profile_picture(jwt_token: str, profile_pic_file: UploadFile = File(...)):
    ...
    await caches.get("profiles").delete(f"profiles_{user_id}")  # Clear the cache to reflect updated profile picture
    #Clear the cache for get all profiles
...
#@cache(namespace="profile", expire=3600)  # Cache the response for 1 hour (3600 seconds)
@cached(ttl=3600, cache=Cache.MEMORY, alias="profiles", key_builder=lambda f, *args, **kwargs: f"all_profiles")
